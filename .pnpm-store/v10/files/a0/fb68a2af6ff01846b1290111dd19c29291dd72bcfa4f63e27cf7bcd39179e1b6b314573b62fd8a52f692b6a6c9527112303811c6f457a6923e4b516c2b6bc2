 ...
 */
type ApiForModule<ModulePath extends string, Module extends object, LastPathWasSegmented extends boolean> = ModulePath extends SegmentedPath<infer First, infer Rest> ? {
 ...
 * @public
 */
export type ApiFromModules<AllModules extends Record<string, object>> = keyof AllModules & SegmentedPath extends never ? ApiFromNonSegmentedModules<AllModules> : ApiFromSegmentedModules<AllModules>;
type ApiFromNonSegmentedModules<AllModules extends Record<string, object>> = {
    [ModulePath in keyof AllModules as keyof FunctionReferencesInModule<AllModules[ModulePath]> extends never ? never : ModulePath]: FunctionReferencesInModule<AllModules[ModulePath]>;
};
type ApiFromSegmentedModules<AllModules extends Record<string, object>> = ExpandModulesAndDirs<IntersectUnionProperties<{
    [ModulePath in keyof AllModules as FirstSegment<ModulePath>]: ModulePath extends SegmentedPath<string, infer Rest> ? ApiForModule<Rest, AllModules[ModulePath], true> : ApiForModule<ModulePath & string, AllModules[ModulePath], false>;
}>>;
type IntersectUnionProperties<O extends object> = {
    [K in keyof O]: UnionToIntersection<O[K]>;
};
type SegmentedPath<First extends string = string, Rest extends string = string> = `${First}/${Rest}`;
type FirstSegment<ModulePath> = ModulePath extends SegmentedPath<infer First> ? First : ModulePath;
/**
 * @public
 *
...
export type FilterApi<API, Predicate> = {
    [mod in keyof API as API[mod] extends Predicate ? mod : API[mod] extends AnyFunctionReference ? never : {} extends FilterApi<API[mod], Predicate> ? never : mod]: API[mod] extends Predicate ? API[mod] : FilterApi<API[mod], Predicate>;
} & unknown;
/**
 ...
 */
type ExpandModulesAndDirs<ObjectType> = ObjectType extends Record<string, AnyFunctionReference> ? ObjectType : {
    [Key in keyof ObjectType as keyof ExpandModulesAndDirs<ObjectType[Key]> extends never ? never : Key]: ExpandModulesAndDirs<ObjectType[Key]>;
};
/**
