...
- **The sender is the namespace; the topic rides in the body.** Live-device finding: the phone assistant surfaces a notification's *sender* and body when reading aloud — never its title, so a generic sender name strips all routing context from the audio surface. Routing lives in the sender at *namespace* granularity: every thread in a project speaks as one stable contact (e.g. `claude-comms`), with the thread topic prepended to the body so the assistant still voices it. Stable contact identity is the point — it keeps the audio surface consistent, and it opens the future move of *initiating* work by voice: telling the assistant to send a new message to the namespace, spawning a fresh thread with no screen involved (parked, not yet built).
