...
- **The sender is the namespace; the topic rides in the body.** Live-device finding: the phone assistant surfaces a notification's *sender* and body when reading aloud — never its title, so a generic sender name strips all routing context from the audio surface. Routing lives in the sender at *namespace* granularity: every thread in a project speaks as one stable contact (e.g. `claude-comms`), with the thread topic prepended to the body so the assistant still voices it. Stable contact identity is the point — it keeps the audio surface consistent, and it opens the move of *initiating* work by voice (below).
- **Initiating by voice rides the reply channel — the lobby.** The assistant platform offers no way to *send a new message* to an app by voice today (the old Assistant path is legacy; its successor — apps exposing tools directly to the assistant — is not yet public). What the assistant can do is reply to notifications, so initiation is smuggled through that: each namespace keeps a standing, silent "lobby" notification in the shade, and replying to it spawns a fresh thread instead of continuing one. Live-device finding: the user addresses a reply target by the conversation's *name*, and the assistant resolves it — so the lobby carries its own distinct, speakable name and dictation lands where intended even with live threads from the same namespace on screen. The name is simply **"Lobby"**: namespace identity already rides the sender, so the title needn't repeat it — the layers compose ("the Lobby from claude-comms") rather than duplicate. (Untested edge: with lobbies from *several* namespaces standing at once, whether the assistant disambiguates on the sender correctly is an on-device check.) The lobby is dismissible but resurrected (swiping it away is a momentary gap, not a loss), and it re-arms after each use. The spawned thread echoes the dictated line as a *silent* card — the user's own words must never alert them (live finding: an alerting echo chimes your own message back at you); the card exists only as the anchor the assistant's response later appends to and alerts on. When the platform's app-tools surface goes public, direct initiation becomes the proper home and the lobby retires.
